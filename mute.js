module.exports = {
  name: 'mute',
  description: 'Mute a user by mention or ID',
  execute(message, args) {
    ...
    if (args.length < 1) {
      return message.reply('Please mention a user or provide a user ID to mute.');
    }
    ...
    member.roles.add(mutedRole)
      .then(() => {
        message.reply(`Successfully muted user with ID: ${user}`);
      })
      .catch(error => {
        console.error(`Error muting user: ${error}`);
        message.reply('There was an error while trying to mute the user.');
      });
  },
};
